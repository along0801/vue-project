      ...
      <div class="mui-card-content">
        <div class="mui-card-content-inner">
          ...
          <p>
            ...
            <!-- 1. 经过分析发现：按钮属于 goodsinfo 页面，数字 属于 numbox 组件 -->
...
export default {
  data() {
    return {
      ...
      selecteCount: 1 // 保存用户选中的商品数量，默认，认为用户买一个
    };
  },
  created() {
    this.getLunbotu();
  ...
  methods: {
    ...
    enter(el, done) {
      ...
      // 小球动画思路：
      // 1. 先分析导致 动画 不准确的 本质原因： 我们把小球 最终 位移到的 位置，已经局限在了 某一分辨率下的 滚动条未滚动的情况下
      // 2. 只要分辨率和 测试的时候不一样，或者 滚动条有一定的滚动距离之后，问题就出现了
      // 3. 因此，我们经过分析，得到结论：不能把 位置的 横纵坐标 直接写死了，而是应该 根据不同情况，动态计算这个坐标值
      // 4. 经过分析，得出解题思路：先得到 徽标的 横纵坐标，再得到 小球额 横纵坐标，然后 让 y 值 求差，x 值也求差，得到的结果就是 横纵坐标要位移的距离
      // 5. 如何获取 徽标 和小球的 位置？？？  domObject.getBoundingClientRect()

      // 获取小球在页面中的位置
      const ballPosition = this.$refs.ball.getBoundingClientRect();
      // 获取徽标在页面中的位置
      // #Tim 注意：在 vue 组件中使用 DOM 操作元素的时候，可以跨组件获取到整个页面的其他组件的元素，不需要通过 父组件传值的形式
    ...
    getSelecteCount(count) {
      ...
      console.log("父组件拿到的数量为：" + this.selecteCount);
    }
  },
  components: {
    swiper, // key 和 value 相同，只写一个
    numbox
  }
};
</script>
